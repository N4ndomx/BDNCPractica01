...
public class DAOEmpleados implements IDAO<PojoEmpleado> {
    ...
    private static final String INSERT_EMPLEADOS_SQL = "INSERT INTO EMPLEADOS "
            + "  ( nombre, telefono, direccion, idDepartamento ) VALUES " + " ( ?, ?, ?, ?);";
    private static final String SELECT_EMPLEADOS_SQL = "SELECT * FROM EMPLEADOS ";
    private static final String UPDATE_EMPLEADOS_SQL = "UPDATE EMPLEADOS SET  nombre = ?, telefono = ?, direccion = ?,"
    ...
    public boolean guardar(PojoEmpleado p) {
        ...
        try {
            ...
            insertSt.setInt(4, p.getIdDepartamento());
    ...
    public PojoEmpleado buscarById(long id) {
        ...
        try {
            PreparedStatement insertSt = cnx.getPreparedStatement(SELECT_EMPLEADOS_SQL + " WHERE id = ?;");
            insertSt.setInt(1, (int) id);
            ResultSet resultSet = insertSt.executeQuery();
            if (resultSet.next()) {
                int clave = resultSet.getInt("id");
                String nombre = resultSet.getString("nombre");
